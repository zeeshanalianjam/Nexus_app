  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop:blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        <img src={selectedUser?.profilePicture || assets.avatar_icon} alt="profile" className='w-8 rounded-full' />
        ...
        {messages.map((message, index) => (
          <div key={index} className={`flex items-end gap-2 justify-end ${message.senderId !== authUser._id && 'flex-row-reverse'}`}>
            ...
            <div className='text-center text-xs'>
              <img src={message.senderId === authUser._id ? authUser?.profilePicture || assets.avatar_icon : selectedUser?.profilePicture || assets.avatar_icon} alt="profile" className='w-7 rounded-full' />
